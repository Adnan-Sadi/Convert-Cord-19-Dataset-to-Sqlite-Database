...
def process_file(file):
    ...
    ref_entries of the paper and returns the information as a dataframe.
    ...
    Returns
    -------
    df_temp : a dataframe the ref_entries data of the paper.
    ...
    df_temp = pd.DataFrame.from_dict(data['ref_entries'], orient='index')
    df_temp['paper_id'] = paper_id
    
    return df_temp

def main():
    ref_entries_df = pd.DataFrame()
    
    print('Fetching all ref entry data')
    print('-----------------------------------------------------------------')
    ...
    for count, chunk in enumerate(chunks):
        ...
        ref_entries_df = pd.concat(results)
    
        #reset and rename index to refent_id
        ref_entries_df.reset_index(inplace=True)
        ref_entries_df = ref_entries_df.rename(columns = {'index':'refent_id'})
    
        ref_entries_df.to_csv(f'D:\\Datasets\\Cord-19\\DataFrames\\ref_entries\\ref_entries_df_pmc_{count}.csv', index=False)
